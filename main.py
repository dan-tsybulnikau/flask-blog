...
def register():
    ...
    if register_form.validate_on_submit():
        user = User(
            ...
            password=generate_password_hash(register_form.password.data, method='pbkdf2:sha256', salt_length=10),
...
def login():
    ...
    if login_form.validate_on_submit():
        ...
        user_in_database = User.query.filter_by(email=login_form_email).first()
        if not user_in_database:
            flash('That email does not exist, try again!')
        else:
            if check_password_hash(user_in_database.password, login_form_password):
...
def show_post(post_id):
    ...
    return render_template("post.html", post=requested_post, admin=is_admin(), form=comment_form)
